...
GameValue KnownTargets(const GameState* state, GameValuePar oper1)
{
    ...
    if (!grp)
    {
        return value; // empty array (mirrors GrpUnits' graceful-empty behaviour)
    }
    ...
    for (int i = 0; i < list.Size(); i++)
    {
        ...
        if (!tgt || !tgt->IsKnown())
        {
            continue; // skip null/stale entries and purely potential contacts
        }
        ...
        // last reported position as SQF [x, y, z] (engine X, Z, Y), matching ObjGetPos
        ...
        // FadingSideAccuracy() matches what GrpKnowsAbout returns, so knownTargets
        // and knowsAbout agree on the "knowledge" number for the same contact.
