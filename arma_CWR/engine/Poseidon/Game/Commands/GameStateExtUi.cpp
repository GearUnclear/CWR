...
GameValue ObjDisableAI(const GameState* state, GameValuePar oper1, GameValuePar oper2)
{
    ...
    // GetEnumValue returns -1 (not INT_MIN) for an unknown name; guard with s < 0
    // so an unrecognised feature is a no-op (dai | 0 == dai) instead of setting
    // every disable bit (dai | -1 == -1). Matches ObjEnableAI's guard.
    if (s < 0)
    {
...
GameValue ObjEnableAI(const GameState* state, GameValuePar oper1, GameValuePar oper2)
{
    ...
    int dai = unit->GetAIDisabled();
    // GetEnumValue returns -1 (not INT_MIN) for an unknown name; guard with s < 0
    // so an unrecognised feature is a no-op (dai & ~0 == dai) instead of clearing
    // every disable bit (dai & ~(-1) == 0).
    if (s < 0)
    {
        s = (AIUnit::DisabledAI)0;
    }
