...
The new command — working name **`nearestRoads <pos> <radius>`** (returns road-segment positions/directions) — is what lets checkpoint placement, road-cut berms, and convoy road-snapping work on *any* island automatically, which the swappable-islands core requirement demands. It joins the evaluator-QoL list in plan 13's Phase 1.5. *(Landed 2026-08-22 with the ambient-traffic service: `<pos> nearestRoads <radius>` / `gmRoadsNear [pos, radius]` return unlocked road-segment centre positions, plus `gmRoadNearest <pos>` and `gmRoadPath [from, to]`; `engine/Poseidon/Game/Guerrilla/TrafficCommands.cpp`.)*
