...
- **Evaluator quality-of-life commands** — `setVariable`/`getVariable`, `isNil`, `nearestObjects`, and whatever else Phase 1 had to route around. (`doMove`/`commandMove` turned out to be registered all along via the `TABLE_COMMAND` table in `GameStateExt.cpp` — asserted by `test_game_state_ext.cpp`.) Each is in the ~1–2 h class (plans 09/10 are the template); collectively they retire the parallel-array/scratch-global idioms.
