...
- The LoBo tests need three one-time `@LoBo` repairs, all idempotent and all rerun after any @LoBo reinstall:
  - `tools/lobo/fix-lobo-scope.ps1` — `LoBoWreck.pbo` and `LoBoPalObj.pbo` are the only two @LoBo addons that omit the `#define private 0 / protected 1 / public 2` header every other @LoBo config carries, so their `scope = public;` resolved to nothing and read back as 0. Eighteen finished wreck and poster classes were refused with `Cannot create '<class>': type is abstract` until this rewrote the value in place. `scripting/prop_spawn_safety` CASE 2b goes red if it has not been run.
  - `tools/lobo/fix-lobo-model-origin.ps1` — `LoBo_M60A1_wreck` and `LoBo_M60A1_wreck2` were authored with their model origin **above** the mesh, and a static prop is seated at `terrainY + shape->BoundingCenter().Y` (`Entity::PlaceOnSurface`, Static branch, `World/Simulation/Simul.cpp:1300`), so `createVehicle [x, y, 0]` buried both tanks whole (roof 0.45 m / 0.92 m under the sand, belly 3.39 m / 3.19 m under). This is **not** a missing LOD: no LoBoWreck model has a LandContact LOD, including the eight that seat correctly. The script rewrites `boundingCenter.Y` in the two p3ds in place (4 bytes each, no repack) and discovers its own targets, so it needs no edit when another buried model turns up. `scripting/prop_spawn_safety` CASE 2d goes red, and the `p03_m60a1_wreck` / `p03b_m60a1_wreck2` probe frames come back as empty desert, if it has not been run.
  - `tests/fixtures/mods-lobo/@lobofixup/gen-patched-pbos.ps1` — stock `@LoBo` ammo pbos hard-abort under `--autotest` on malformed `tracerColor` floats (`0.0.1`). On this machine `@LoBo`'s own `LoBoammo.pbo`/`LoBo_airammo.pbo` were already repaired in place on 2026-07-16 and are byte-identical to the fixture's shadows, so the shadows are currently inert; the fixture's `bin/config.cpp` (its `CfgAddons` preload roster and `CfgGuerrillaFactions`) is what still carries the lane.
