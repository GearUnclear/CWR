...
| **Journal** (native) | the map screen's notepad as the commander's diary, in the notepad's stock look (typed Courier, handwritten entries in ink): Notes (the day's page), Plan (objectives + next moves), Zones index + a page per zone, Cell, Resistance, Diary, Handbook; fed by the scripts through `gmJournal*` (`gmJournalNote` tags a line with its zone and kind), serialized as `GuerrillaJournal` | `engine/Poseidon/Game/Guerrilla/Journal.*` + `UI/Guerrilla/GuerrillaJournalPages.*` |
