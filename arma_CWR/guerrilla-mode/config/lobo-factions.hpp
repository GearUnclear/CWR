...
// Hizballah, PLO and PLO_East - the GLOBAL half of the Guerrilla faction table
// for the Lost Brothers mod (issue #54 A4, given a player-facing home by #56
// task 2).
//
// This is the ONE tracked copy. Two consumers include or install it:
//
//   * tools/lobo/install-lobo-factions.ps1 writes it into
//     <@LoBo>\bin\config.cpp, the mod's own deferred-merge config, so a player
//     who mounts @LoBo gets the eight rosters on every island with no test
//     fixture mounted. @LoBo is gitignored third-party data, so the install is
//     a regeneration step (rerun after a reinstall), like the mod doctor.
//   * tests/fixtures/mods-lobo/@lobofixup/bin/config.cpp #includes it, so the
//     lobo test lanes carry the same bytes whether or not the install ran.
//
// The engine unions this table with each island template's own
// CfgGuerrillaFactions block (Game/Guerrilla/FactionSources.*), the island
// winning on a class-name collision. The CIV descriptor stays island-owned by
// rule and must never appear here: it names the population models and
// ambient-traffic hulls a given island's data set ships. Key schema:
// guerrilla-mode/config/guerrilla-factions.hpp and FACTION-PACKS.md.
//
// Every class this file names is validated by
//   PoseidonTools guerrilla lint --data-dir "<Classic>" --mod "<@LoBo>"
// which must report the eight rosters with zero unresolved keys.
