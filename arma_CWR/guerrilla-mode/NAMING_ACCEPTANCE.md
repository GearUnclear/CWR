...
| Names recur in game and persist after death in records/memorials | `BindRow` sets the live body's name; journal gathering and the persisted companion status line read the same registry display name. Registry tests cover awards, death, retained deeds, reload and correction of a stale startup roster without another character event; journal compose tests cover rosters, memorials and records keyed by character id. The naming integration lane now checks the real memorial and fallen dossier. | Run that lane and inspect labels, roster and memorial rendering. |
