...
| 22 | Field journal on the map screen: Situation / Plan / Zones (+ a page per zone) / Cell / Resistance / Diary / Handbook | engine `Game/Guerrilla/Journal` (+ `JournalCommands.cpp`: `gmJournalLog` / `gmJournalNote` / `gmJournalObjective` / `gmJournalStatus` / `gmJournalCount` / `gmJournalEntry` / `gmJournalObjectiveState` / `gmJournalStatusText` / `gmIslandName` / `gmDisplayName`) + `UI/Guerrilla/GuerrillaJournalPages` (theme + page renderer hooked into `DisplayMap::ReloadBriefingContent`) + `UI/Controls` HTML-control extensions (per-field colour, solid bars, hanging indent) + `scripts/` diary/objective/status writes | **native + script** | Redesigned 2026-09-05 (issue #58): a dark, one-grid journal on the notepad drawn by the stock briefing control through small `CHTMLContainer` extensions; pages SITUATION (`Main`), PLAN, ZONES index + `GM_ZONE_<i>` per zone with that zone's own diary lines and a last-seen stamp (new `ZoneRecord::seenDay/seenMinute`, saved), CELL (native roster: player's group + holding squads, arms, supply), RESISTANCE (war-level ladder, ground, occupier order of battle, faction-management stubs), DIARY, HANDBOOK (ten in-universe chapters); a RED alert strip and a bottom-pinned nav row on every page. Diary entries carry a zone tag + kind (`gmJournalNote`), scripts write in log voice, loot publishes `Standard issue` / `Next pattern`. Design iterated against an adversarial UI auditor on a render-model mock (rounds 1-3: 6 → 7 → 8 /10; see issue #58). **Code-complete on Linux (unit lane), not yet built or play-tested on the Windows/Classic lane**: the theme's fonts/sizes and the notepad's real page height need an in-game pass |
...
| 26b | Unassociated assailants: civilians who fight back, rogues hostile to both sides (#42) | *not implemented* | **planned (engine + script)** | The `GM_CI_ON_RESIST` callback row 26 emits is still a no-op. Plan: a computed third-force war side (the one neither campaign side uses), welded hostile to everyone once per campaign load beside the existing friendship weld, with a capped pool of assailant bodies created onto it. See [UNASSOCIATED-ASSAILANTS.md](UNASSOCIATED-ASSAILANTS.md), roadmap `unassociated-assailants`. |
...
| *(engine)* `Game/Guerrilla/Journal.*`, `JournalCommands.cpp`, `UI/Guerrilla/GuerrillaJournalPages.*` | the field journal behind the map's notepad (row 22): diary / objectives / status tables (`gmJournal*`, `gmDisplayName`), the theme + page renderer + handbook text; `scripts/` only WRITE to it (campaign: opened/saved/restored; capture: lost/ready/risen/liberated tagged by zone; qrf: RED + QRF out; undercover: cover blown; companions: promotion/death + `Companions` status line; loot: pattern in issue + `Standard issue` / `Next pattern` status lines + `firstUnlock` objective; escalation: war-level edges; recruit: recruit/train + `firstRecruit` objective; market: HQ / garage / cache / purchases tagged by zone) |
...
| `scripting/guerrilla_journal_pages.test` | `full_cwa` (opens the real map display: Situation/Plan/Zones + zone pages/Cell/Resistance/Diary/Handbook, link routing, live repaint of a tagged note) - **not yet re-run after the 2026-09-05 redesign** |
