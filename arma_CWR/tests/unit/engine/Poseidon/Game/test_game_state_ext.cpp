...
TEST_CASE("enableAI clears the disable bits that disableAI sets", "[game][gameStateExt][ai]")
{
    // Lock in the enum-name mapping that the disableAI/enableAI operators rely on
    // (DisabledAINames[] in AIUnit.cpp). These are the only accepted feature names.
    REQUIRE(Foundation::GetEnumValue<AIUnit::DisabledAI>("MOVE") == AIUnit::DAMove);
    REQUIRE(Foundation::GetEnumValue<AIUnit::DisabledAI>("TARGET") == AIUnit::DATarget);
    REQUIRE(Foundation::GetEnumValue<AIUnit::DisabledAI>("AUTOTARGET") == AIUnit::DAAutoTarget);
    REQUIRE(Foundation::GetEnumValue<AIUnit::DisabledAI>("ANIM") == AIUnit::DAAnim);

    // An unknown name maps to -1 (NOT INT_MIN); the enableAI guard (s < 0) depends
    // on this so a typo is a no-op rather than clearing every disable bit.
    REQUIRE(Foundation::GetEnumValue<AIUnit::DisabledAI>("BOGUS") == -1);

    // Mirror the operator-table mask logic exactly:
    //   disableAI: dai | s,  s < 0 -> 0     (ObjDisableAI, GameStateExtUi.cpp)
    //   enableAI:  dai & ~s, s < 0 -> 0     (ObjEnableAI,  GameStateExtUi.cpp)
    auto disableBit = [](int dai, const char* name)
    {
        AIUnit::DisabledAI s = Foundation::GetEnumValue<AIUnit::DisabledAI>(name);
        if (s < 0)
        {
    ...
    auto enableBit = [](int dai, const char* name)
    {
        AIUnit::DisabledAI s = Foundation::GetEnumValue<AIUnit::DisabledAI>(name);
        if (s < 0)
        {
    ...
    // Round-trip: disabling then enabling the same feature restores a clean mask.
    int mask = 0;
    mask = disableBit(mask, "MOVE");
    REQUIRE(mask == AIUnit::DAMove);
    mask = enableBit(mask, "MOVE");
    REQUIRE(mask == 0);

    // Enabling one feature clears only its bit and leaves the others disabled.
    mask = 0;
    mask = disableBit(mask, "MOVE");
    mask = disableBit(mask, "AUTOTARGET");
    REQUIRE(mask == (AIUnit::DAMove | AIUnit::DAAutoTarget));
    mask = enableBit(mask, "AUTOTARGET");
    REQUIRE(mask == AIUnit::DAMove);
    mask = enableBit(mask, "MOVE");
    REQUIRE(mask == 0);

    // An unknown feature name must be a no-op for enableAI (must not wipe the mask).
    mask = AIUnit::DAMove | AIUnit::DATarget | AIUnit::DAAutoTarget | AIUnit::DAAnim;
    REQUIRE(enableBit(mask, "BOGUS") == mask);
}

TEST_CASE("enableAI and knownTargets are registered in GGameState", "[game][gameStateExt][ai]")
{
    GGameState.Reset();
    Poseidon::Foundation::InitModules();
...
TEST_CASE("Guerrilla zone-registry commands are registered in GGameState", "[game][gameStateExt][guerrilla]")
{
    ...
    REQUIRE(ContainsName(nulars, "gmOccupierSide"));
    REQUIRE(ContainsName(nulars, "gmResistanceSide"));
