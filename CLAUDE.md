...
- **`ARMA Cold War Assault [Classic]/`** — original v1.99 game data, ignored by Git. Prepare a fresh copy with `arma_CWR/setup-guerrilla.ps1` (see `arma_CWR/guerrilla-mode/FRESH-INSTALL.md`). Classic compatibility/menu shims now live in `guerrilla-mode/runtime/classic`, Options text resources under separately licensed `thirdparty/classic-ui`, and OFL fonts under `thirdparty/guerrilla-fonts`. Edit those tracked sources and reinstall; do not rely on old local-only overrides or renamed Windows fonts. Base game PBOs remain external.
