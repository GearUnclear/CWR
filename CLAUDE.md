...
- **`Arma Cold War Assault Demo [Remaster]/`** — the official remaster Demo package (Steam app 4819000). **Read-only reference, like `@LoBo` — never edit.** Three roles: (1) data source for the `demo`-tagged integration tests; (2) contract source — its `BIN/remaster.cpp` inventories every `Remaster >>` config key the engine reads, the checklist for backfilling the Classic shim; (3) reference engine build — `PoseidonGameDemo.exe` is the official binary of this repo's remaster engine, for "ours or upstream's?" behaviour diffs (#11/#13).
