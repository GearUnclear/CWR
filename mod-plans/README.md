...
Code-only modification plans for the **Poseidon** engine (Arma: Cold War Assault, originally Operation Flashpoint). Every plan here is implementable **without any art assets** — no models, textures, sounds, or missions. They touch only engine/game C++ and the SQF scripting surface.
...
| --- | --- | --- | --- | --- |
| [01](./01-ai-squad-formations.md) | New & retuned squad formations | ai-tactics | beginner | Adds combat-spread, herringbone, and diamond formations (and retunes existing ones) by editing one hardcoded geometry table plus three small enum/name sites. |
...
| [11](./11-sim-bullet-penetration.md) | Bullet penetration through soft cover | sim | intermediate | Generalizes the hardcoded glass pass-through in `ShotShell::Simulate` into a material-driven penetration model (wood, foliage, thin metal) at reduced damage. |
...
| [13](./13-guerrilla-mode.md) | Guerrilla mode (open-world insurgency) | game-mode | project | Master plan for a persistent open-world guerrilla game — grow & train a faction, capture occupied towns, escalating enemy. ~90% SQF over the engine; consumes plans 01–10 as dependencies. |

## Recommended focus: AI

Plans **01–08** are the AI core (tactics + perception) and are the recommended primary focus. Plans 09–10 (scripting) double as **probes/test hooks** for the AI work, and 11–12 (sim + dev tooling) support and validate it. Plan **13** is the umbrella **game-mode** plan — a persistent open-world guerrilla insurgency that *consumes* the AI plans (02–05/08) and scripting commands (09/10) as dependencies; read it last, as the application that ties the engine work to a game.
...
2. **[01] Squad formations** — one hardcoded table + enum + name. Learn how AI data flows without touching control flow.
3. **[06] Memory decay** — three fade curves moved to config, verifiable via `knowsAbout`. Learn the config plumbing.
4. **[09] `enableAI`** — a near-copy of `disableAI`. Learn the command-registration pattern.
5. **[10] `knownTargets`** — learn how engine state is marshalled back to SQF. (09 + 10 give you scripting probes for the harder work.)
6. **[02] Dynamic morale** and **[04] Target priority** — single scoring functions, localized intermediate behavior.
7. **[03] Flee to cover** and **[07] Stance/concealment spotting** — reuse the exposure-cost and visibility machinery from above.
8. **[11] Bullet penetration** — learn the projectile collision loop; also the mental model for suppression's near-miss source.
9. **[05] Working suppression** (flagship) and **[08] Shared enemy intel** — capstones that tie multiple subsystems together. The diagnostics tab and the `knowsAbout`/`knownTargets` probes feed directly into building and verifying these.
